...
function buildChrome() {
  document.getElementById("site-header").innerHTML = `
    <div class="wrap header-inner">
      ...
      <button class="nav-toggle" aria-label="Menú" onclick="document.body.classList.toggle('nav-open')">☰</button>
      <nav class="nav">
        <a data-nav="home" href="#/home">Inicio</a>
        <a data-nav="cursos" href="#/cursos">Cursos</a>
        <a class="nav-ext" href="${CONFIG.URL_ALUMNOS}" target="_blank" rel="noopener">Área del alumno ↗</a>
        <a class="nav-ext" href="${CONFIG.URL_PANEL}" target="_blank" rel="noopener">Sistema ↗</a>
        <a class="btn btn-primary btn-sm" href="#/inscripcion">Inscribite</a>
      </nav>
    </div>`;

  const waHref = CONFIG.WHATSAPP_NUMBER ? `https://wa.me/${CONFIG.WHATSAPP_NUMBER}` : null;
  const contactos = [
    waHref
      ? `<a class="foot-link" href="${waHref}" target="_blank" rel="noopener">💬 WhatsApp</a>`
      : "",
    CONFIG.TELEFONO
      ? `<a class="foot-link" href="tel:${CONFIG.TELEFONO.replace(/\s/g, "")}">📞 ${esc(CONFIG.TELEFONO)}</a>`
      : "",
    CONFIG.EMAIL_CONTACTO
      ? `<a class="foot-link" href="mailto:${CONFIG.EMAIL_CONTACTO}">✉️ ${esc(CONFIG.EMAIL_CONTACTO)}</a>`
      : "",
    CONFIG.INSTAGRAM_URL
      ? `<a class="foot-link" href="${CONFIG.INSTAGRAM_URL}" target="_blank" rel="noopener">📸 Instagram</a>`
      : "",
    CONFIG.DIRECCION ? `<span class="foot-link muted">📍 ${esc(CONFIG.DIRECCION)}</span>` : "",
  ]
    .filter(Boolean)
    .join("");
    ...
    <div class="wrap foot-inner">
      ...
      <div class="foot-cols">
        <div>
          ...
          <a class="foot-link" href="#/home">¿Por qué OBE?</a>
...
async function viewHome() {
  ...
  const ctaSecundario = wa
    ? `<a class="btn btn-wa" href="${wa}" target="_blank" rel="noopener">💬 Consultá por WhatsApp</a>`
    : `<a class="btn btn-ghost" href="#/inscripcion">Solicitá información</a>`;

  app.innerHTML = `
    <section class="hero">
      <div class="wrap hero-inner">
        <div class="hero-copy reveal">
          <span class="pill">🎓 Formación profesional · Certificado incluido</span>
          <h1>Estudiá una <span class="hl">carrera profesional</span></h1>
          <p class="lead">
            Cursos con certificado, pensados para el mercado laboral paraguayo.
            Pagás cómodamente <strong>por carné, en cuotas</strong>.
          </p>
          <div class="hero-cta">
            <a class="btn btn-primary" href="#/cursos">Ver los ${cursos.length} cursos</a>
            ${ctaSecundario}
          </div>
    ...
    <section class="wrap section" id="por-que">
      ...
      <div class="grid features">
        ${WHY.map(
          (f) => `<article class="feature reveal"><div class="feature-ic">${f.ic}</div>
    ...
    <section class="strip-alt">
      <div class="wrap section">
        <div class="section-head reveal">
          ...
          <a class="link-more" href="#/cursos">Ver todos →</a>
        </div>
        <div class="grid cards">
          ${cursos.map((c) => `<div class="reveal">${cardCurso(c)}</div>`).join("")}
        </div>
      </div>
    </section>
    ...
    <section class="cta-band">
      <div class="wrap cta-inner reveal">
        ...
        <div class="cta-actions">
          ...
          ${wa ? `<a class="btn btn-white btn-lg" href="${wa}" target="_blank" rel="noopener">💬 WhatsApp</a>` : ""}
        </div>
      </div>
    </section>`;
}
...
const WHY = [
  { ic: "📜", t: "Certificado incluido", d: "Recibí tu certificado al finalizar el curso." },
  { ic: "🧾", t: "Pago por carné", d: "Cuotas accesibles en Guaraníes, sin tarjeta ni pago online." },
  { ic: "👩‍🏫", t: "Docentes del área", d: "Aprendé con profesionales que enseñan lo que hacen." },
  { ic: "💻", t: "Formación práctica", d: "Contenido orientado al mercado laboral paraguayo." },
  { ic: "🕒", t: "Horarios flexibles", d: "Cursos presenciales y a distancia, a tu ritmo." },
  { ic: "🤝", t: "Acompañamiento", d: "Te guiamos desde la inscripción hasta el certificado." },
];
...
function testimoniosSection() {
  ...
  if (!t.length) return "";
  return `<section class="strip-alt"><div class="wrap section">
    <div class="section-head center reveal">
...
async function viewCatalogo() {
  ...
  app.innerHTML = `
    <section class="wrap section">
      <div class="page-head">
        ...
        <p class="muted">${cursos.length} cursos de formación profesional a distancia.</p>
      </div>
      <div class="toolbar">
        <input id="buscar" class="input" type="search" placeholder="Buscar curso…" autocomplete="off" />
        <span id="contador" class="muted"></span>
      </div>
      <div id="lista" class="grid cards">${cursos.map(cardCurso).join("")}</div>
  ...
  const upd = () => {
    ...
    const filtrados = cursos.filter((c) => (c.nome || "").toLowerCase().includes(q));
  ...
  contador.textContent = `${cursos.length} cursos`;
}
...
async function viewCurso([id]) {
  ...
  const meta = [
    c.duracao_meses ? `⏳ ${c.duracao_meses} meses` : null,
    c.carga_horaria ? `📚 ${c.carga_horaria} h` : null,
    `🖥️ ${modalidadLabel(c.modalidade)}`,
    c.qtd_certificados ? `📜 ${c.qtd_certificados} certificado(s)` : null,
  ].filter(Boolean);
    ...
    <section class="wrap curso-detalle">
      <div class="curso-main">
        <div class="curso-hero">
          <div class="curso-ic">${cursoIcon(c.nome)}</div>
          <div>
            <h1>${esc(c.nome)}</h1>
      ...
      <aside class="curso-side">
        <div class="price-card">
          ...
          <div class="price-note">🧾 Pago por carné, en cuotas · Guaraníes</div>
  ...
  return `
    <a class="card curso-card" href="#/curso/${encodeURIComponent(c.id)}">
      <div class="curso-card-top">
        <span class="curso-card-ic">${cursoIcon(c.nome)}</span>
        <span class="tag">${modalidadLabel(c.modalidade)}</span>
      </div>
      <h3>${esc(c.nome)}</h3>
